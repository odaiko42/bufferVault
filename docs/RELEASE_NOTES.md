...
- **constants.rs** : reorganisation en 5 categories numerotees, doc-comments avec refs NIST/OWASP/RFC, 9 assertions compile-time (`const _: ()`) pour invariants crypto/Win32
- **clipboard/injector.rs** : extraction des helpers purs `computeAllocSize()` / `makeKeyInput()`, 3 constantes nommees (`PASTE_INPUT_COUNT`, `PASTE_DELAY_MS`, `UTF16_BYTE_SIZE`), commentaires SAFETY detailles avec invariants
- **clipboard/mod.rs** : helper pur `isAnyTextFormat()`, re-exports des types courants, unsafe isole par appel FFI
- **clipboard/monitor.rs** : helper pur `computeU16Count()`, constante `UTF16_BYTE_SIZE`, sections numerotees 1-7, annotations `# Platform` sur toutes les fonctions publiques et internes
- **config/mod.rs** : ajout de `validateConfig()` avec validation des seuils de securite (PBKDF2, taille d'entree), re-exports des types courants, constantes `MIN_PBKDF2_ITERATIONS` / `MIN_ENTRY_SIZE`
- **config/parser.rs** : doc-comments enrichis avec `# Arguments` / `# Returns` / `# Gestion d'erreur` sur toutes les fonctions publiques et privees, sections numerotees 1-7
...
- **663 tests unitaires** (vs 484 en v0.1.2), 0 warnings

### Nouveaux tests (179 nouveaux tests)
...
- `constants.rs` : 29 tests (0 -> 29) -- invariants crypto (AES, GCM, PBKDF2, HMAC, SHA256), vault magic, Win32 IDs, filenames, defaults
- `clipboard/injector.rs` : 18 tests (0 -> 18) -- computeAllocSize, makeKeyInput press/release/padding, INPUT struct size, constantes, integration no-panic
- `clipboard/mod.rs` : 10 tests (3 -> 10) -- isAnyTextFormat all combos, clear double, clear returns result, is_text_available idempotent
- `clipboard/monitor.rs` : 32 tests (20 -> 32) -- computeU16Count, ansi edge cases, file list 3 paths, classify hdrop+ansi, capture/register/unregister null hwnd, utf16 byte size
- `config/mod.rs` : 11 tests (0 -> 11) -- validateConfig default/low pbkdf2/exact/low entry size/both ok/first failure, constantes OWASP, re-exports, settings default coherent
- `config/parser.rs` : 55 tests (42 -> 55) -- parse empty/comments only, strict empty/comments, serialize empty, kv value with equals, multiple sections, quoting empty, bool mixed case, u32/usize zero, string list single, strict error description
